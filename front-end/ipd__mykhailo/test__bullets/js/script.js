...
window.addEventListener('scroll', function ()  {
    navBlockTopPositions=[];
    navBlockBottomPositions=[];
    ...
    for (let i = 0; i < navBlocks.length; i++) {
        ...
        console.log(navBlockBottomPositions[0]);
    }
    for (let i = 0; i < navBlocks.length; i++) {
       if (navBlockTopPositions[i]< 0 && navBlockBottomPositions[i] > 0) {
         
            console.log(navBlocks[i]);
            for (let i = 0; i < nav__bullets.length; i++) {
                nav__bullets[i].classList.remove('nav-bt_active');
